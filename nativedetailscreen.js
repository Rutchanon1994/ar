...
var ServerInformation = {
    PATH_SERVER: "http://192.168.1.117/yantakao/",
    POIDATA_SERVER: "http://192.168.1.117/yantakao/getpoi.php",
    POIDATA_SERVER_ARG_LAT: "lat",
    POIDATA_SERVER_ARG_LON: "lng",
    POIDATA_SERVER_ARG_RAS: "radius"
};
...
var World = {
    ...
    loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiData) {
        ...
        World.markerImageRessources.idle["ร้านอาหาร"] = new AR.ImageResource("assets/food.png", {
            onError: World.onError
        });
        World.markerImageRessources.selected["ร้านอาหาร"] = new AR.ImageResource("assets/food_selected.png", {
            onError: World.onError
        });
        World.markerImageRessources.idle["ร้านสะดวกซื้อ"] = new AR.ImageResource("assets/super.png", {
            onError: World.onError
        });
        World.markerImageRessources.selected["ร้านสะดวกซื้อ"] = new AR.ImageResource("assets/super_selected.png", {
            onError: World.onError
        });
        World.markerImageRessources.idle["สถานที่ท่องเที่ยว"] = new AR.ImageResource("assets/travel.png", {
                    onError: World.onError
        });
                World.markerImageRessources.selected["สถานที่ท่องเที่ยว"] = new AR.ImageResource("assets/travel_selected.png", {
                    onError: World.onError
                });
                World.markerImageRessources.idle["ที่พัก"] = new AR.ImageResource("assets/hostel.png", {
                            onError: World.onError
                        });
                        World.markerImageRessources.selected["ที่พัก"] = new AR.ImageResource("assets/hostel_selected.png", {
                            onError: World.onError
                        });
        World.markerImagebutton = new AR.ImageResource("assets/open.png", {
            onError: World.onError
        });
        ...
        for (var currentPlaceNr = 0; currentPlaceNr < poiData.length; currentPlaceNr++) {
            var singlePoi = {
                ...
                "recommend":poiData[currentPlaceNr].recommend,
                "time":poiData[currentPlaceNr].time,
                "price":poiData[currentPlaceNr].price,
                "type": poiData[currentPlaceNr].type,
                "pic": ServerInformation.PATH_SERVER + "image/" + poiData[currentPlaceNr].pic,
                "video": ServerInformation.PATH_SERVER + "video/" + poiData[currentPlaceNr].video,
                "model": ServerInformation.PATH_SERVER + "3d/" + poiData[currentPlaceNr].model,
            };
    ...
    updateDistanceToUserValues: function updateDistanceToUserValuesFn() {
        for (var i = 0; i < World.markerList.length; i++) {
            World.markerList[i].descriptionLabel.text = (World.markerList[i].markerObject.locations[0].distanceToUser() > 999) ?
                ((World.markerList[i].markerObject.locations[0].distanceToUser() / 1000).toFixed(2) + " km") :
                (Math.round(World.markerList[i].markerObject.locations[0].distanceToUser()) + " m");
    ...
    locationChanged: function locationChangedFn(lat, lon, alt, acc) {
        ...
        if (!World.initiallyLoadedData) {
            ...
            var actionRange2 = new AR.ActionRange(location1, 50, {
                onExit: function () {
                    actionRange2.enabled = true; //an ActionArea which can only be entered once
                    alert("out of rang");
                    console.log("enter");
                }
            });*/
    ...
    */
    ...
    onMarkerSelected: function onMarkerSelectedFn(marker) {
        ...
        //     World.currentMarker.video.pause();
        //     World.currentMarker.setDeselected(World.currentMarker);
        // });

    },
    showDetail: function showDetailFN(marker) {
        World.currentMarker = marker;
        var list = document.getElementById("img-container");
        var img = new Image();
        img.src = marker.poiData.pic;
        ...
        */
        ...
        $("#poi-detail-recommend").html(marker.poiData.recommend);
        $("#poi-detail-time").html(marker.poiData.time);
        $("#poi-detail-price").html(marker.poiData.price);
    ...
    docheck(checkboxElem) {
        console.log(checkboxElem.value);
        if (checkboxElem.checked && checkboxElem.value == 'ร้านอาหาร') {
            for (var k = 0; k < World.markerList.length; k++) {
                //console.log(World.markerList[k]);
                if (World.markerList[k].poiData.type == 'ร้านอาหาร') {
                    World.markerList[k].setShow(World.markerList[k]);
                }
            }

        } else {
            console.log("notchecked");
            if (checkboxElem.value == 'ร้านอาหาร') {
                for (var k = 0; k < World.markerList.length; k++) {
                    console.log(World.markerList[k]);
                    if (World.markerList[k].poiData.type == 'ร้านอาหาร') {
                        World.markerList[k].setHide(World.markerList[k]);
                    }
                }
            }

        }
        if (checkboxElem.checked && checkboxElem.value == 'ร้านสะดวกซื้อ') {
            for (var k = 0; k < World.markerList.length; k++) {
                //console.log(World.markerList[k]);
                if (World.markerList[k].poiData.type == 'ร้านสะดวกซื้อ') {
                    World.markerList[k].setShow(World.markerList[k]);
                }
            }
            ...
            console.log("notchecked");
            if (checkboxElem.value == 'ร้านสะดวกซื้อ') {
                for (var k = 0; k < World.markerList.length; k++) {
                    console.log(World.markerList[k]);
                    if (World.markerList[k].poiData.type == 'ร้านสะดวกซื้อ') {
                        World.markerList[k].setHide(World.markerList[k]);
        ...
        if (checkboxElem.checked && checkboxElem.value == 'สถานที่ท่องเที่ยว') {
                    for (var k = 0; k < World.markerList.length; k++) {
                        //console.log(World.markerList[k]);
                        if (World.markerList[k].poiData.type == 'สถานที่ท่องเที่ยว') {
                            World.markerList[k].setShow(World.markerList[k]);
                        }
                    }
                    ...
                    console.log("notchecked");
                    if (checkboxElem.value == 'สถานที่ท่องเที่ยว') {
                        for (var k = 0; k < World.markerList.length; k++) {
                            console.log(World.markerList[k]);
                            if (World.markerList[k].poiData.type == 'สถานที่ท่องเที่ยว') {
                                World.markerList[k].setHide(World.markerList[k]);
        ...
        if (checkboxElem.checked && checkboxElem.value == 'ที่พัก') {
                            for (var k = 0; k < World.markerList.length; k++) {
                                //console.log(World.markerList[k]);
                                if (World.markerList[k].poiData.type == 'ที่พัก') {
                                    World.markerList[k].setShow(World.markerList[k]);
                                }
                            }
    ...
    */
    ...
    requestDataFromServer: function requestDataFromServerFn(lat, lon) {
        ...
        var serverUrl = ServerInformation.POIDATA_SERVER + "?" + ServerInformation.POIDATA_SERVER_ARG_LAT + "=" +
            lat + "&" + ServerInformation.POIDATA_SERVER_ARG_LON + "=" + lon + "&" + ServerInformation.POIDATA_SERVER_ARG_RAS + "=200000";
